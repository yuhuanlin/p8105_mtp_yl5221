...
## Problem 2 - EDA
The resulting weight df contains date and lbs weight of each dog and the standard object during observation. There are 361 unique dates.
```{r, results='hide'}
weight_df %>% 
  summarize(distinct_date = n_distinct(date))
```
Summarizing number of observations, average weight and the standard deviation for each dog
```{r}
weight_df %>% 
  pivot_longer(
    raisin:gagne, 
